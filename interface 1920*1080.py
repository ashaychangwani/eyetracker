...
camFrameWidth=width*2/3
menuFrameWidth=width-camFrameWidth

cam1FrameHeight=0.7*height
...
#        model.add(Dropout(0.2))
        ...
        model.fit(X_train,y_train,batch_size=16,epochs=350)
...
#            time.sleep(0.05)
            ...
            pointer=canvas1.create_oval(int(gPredX*width-50), int(gPredY*height-50), int(gPredX*width+50), int(gPredY*height+50), outline="#f11",fill="#1f1", width=2)
...
#                    test=True
        ...
        if(cv2.contourArea(maxC)>3000):
            ...
            row.extend([(x+w/2)/(width/2),(y+h/2)/(height/2)])
        else:
            row=[]
        ...
        if trainingComplete and len(row)==8:
            ...
            print("RESULTANT COORD:",gPredX*width,"  ",gPredY*height)
            
        lmain.after(10, show_frame)
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print("EXCP HERE2",exc_type, fname, exc_tb.tb_lineno)
        print(e)
...
thresholdSlider.place(x=20, y=cam2FrameHeight/2+20, anchor="w")
